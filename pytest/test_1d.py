...
@pytest.mark.parametrize("solver",["sparsecholesky","umfpack","masterinverse"])
def test_1dlaplace(solver):
    m = meshing.Mesh()
    m.dim = 1
    ...
    l.Assemble()
    a.Assemble()
    invmat = a.mat.Inverse(fes.FreeDofs(),inverse=solver)
    ugf.vec.data = invmat*l.vec
    error = sqrt(Integrate( (ugf-uex)*(ugf-uex) ,mesh ))
    assert error <= 1e-14

if __name__ == "__main__":
    test_1dlaplace("umfpack")